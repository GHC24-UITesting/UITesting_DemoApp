...
const Landing = ({ tasks }: LandingProps) => {
  ...
  const items = tasks.map((task) => {
    return { name: task, description: "test_description", selected: false };
  });
    ...
    <div className={styles.main}>
      ...
      <div className={styles.onboardButton}>
        ...
        >
          Onboard{" "}
          {numSelected > 0
            ? `${numSelected} task${numSelected === 1 ? "" : "s"}`
            : ""}
        </Button>
      </div>
              ...
              floatingAction={
                <Checkbox
                  onChange={() => {
                    ...
                    newSelectedTasks[idx].selected =
                      !newSelectedTasks[idx].selected;
                    setSelectedTasks(newSelectedTasks);
                  }}
                  checked={
                    selectedTasks.find((taskObj) => taskObj.name === task)
                      ?.selected ?? false
                  }
                />
              }
              ...
              <CardHeader
                ...
                description={
                  <Caption1 className={styles.caption}>
                    {
                      items.find((taskObj) => taskObj.name === task)
                        ?.description
                    }
                  </Caption1>
                }
