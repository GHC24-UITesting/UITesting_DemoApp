...
describe("getVenueDetails()", () => {
  it("should return the correct venue details", () => {
    expect(getVenueDetails(testVenue1)).toEqual(
      "Phone: +1 212-768-1560, Address: Times Square, New York, NY, USA"
    );
    expect(getVenueDetails(testVenue2)).toEqual(
      "Phone: +1 212-736-3100, Address: Empire State Building, New York, NY, USA"
    );
  });
});
...
describe("filterEventsByTime()", () => {
  it("only returns morning events", () => {
    ...
    expect(filteredEventsNames).toEqual([
